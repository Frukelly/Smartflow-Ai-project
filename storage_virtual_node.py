...
class StorageVirtualNode:
    def __init__(
        ...
        self.bandwidth = bandwidth * 1000000  # Mbps -> bits per second
    ...
    # ===== TELEMETRY METHOD =====
    def get_telemetry(self) -> Dict[str, Union[str, float, int]]:
        """Return node telemetry for monitoring"""
        return {
            "node_id": self.node_id,
            "cpu_used_percent": 100 * (self.total_requests_processed / max(1, self.cpu_capacity)),  # simple simulated metric
            "memory_used_percent": 100 * (len(self.active_transfers) / max(1, self.memory_capacity)),  # simple simulated
            "storage_used_percent": (self.used_storage / self.total_storage) * 100,
            "active_transfers": len(self.active_transfers)
        }
